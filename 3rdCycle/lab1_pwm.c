...
void printPWM(unsigned int pwm_value) {
  unsigned char lcd_buffer[16];
  lcd_gotoxy(0,0);
  sprintf(lcd_buffer,"PWM = %i%%",((pwm_value*100)/255));
  lcd_puts(lcd_buffer);
  delay_ms(10);
}
...
void main() {
  ...
  while (1) {
    ...
    if((PIND.1==0)&&(pwm_value !=0xff)) {
      pwm_value+=1;
      delay_ms(10);
    } // 10ms delay

    if((PIND.0==0)&&(pwm_value !=0x00)) {
      pwm_value-=1;
      delay_ms(10);
    } // 10ms delay

    OCR0A = pwm_value; // PWM filling from 0 to 255
    printPWM(pwm_value);
  }
}