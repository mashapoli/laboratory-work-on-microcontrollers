...
// SPI functions
#include <spi.h>
void MCP4921_Sent_Data (unsigned char data1, unsigned char data2) 
{          
PORTB.2=0; 
SPDR=data1; //отправляем первый байт 
while(!(SPSR & (1<<SPIF))); 
SPDR=data2; //отправляем второй байт 
while(!(SPSR & (1<<SPIF))); 
...
while (1)
      { 
      // Place your code here
      TCNT0=100;TCCR0B=0x05;
      PORTC.1=1;      
      adac_h=0b00110000; //четыре старших бита 0011
      // adac_l=0b11111111; 
      adac_l=read_adc(0);
          
      PORTB.2=1; 
      MCP4921_Sent_Data(adac_h,adac_l); 
